...
const NoteCard: React.FC<NoteCardProps> = ({ note, groups, onUpdate, onDelete, onMoveItem }) => {
  ...
  const handleSave = () => {
    if (title.trim() === "") {
      // Optionally, show a toast or error that title cannot be empty
      setTitle(note.title); // Reset to original title if save is attempted with empty
    }
    onUpdate({ title: title.trim() || note.title, content });
    setIsEditing(false);
  };
          ...
          <CardHeader className="p-4 pb-0 flex flex-row justify-between items-center">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="opacity-0 group-hover/card:opacity-100">
